...
public class MultiServer {
    ...
    public static void main(String args[]) throws IOException
    {
        ...
        {
            String inputline="";
            ArrayList iline= new ArrayList();
            
            while( x<=10 && (inputline=in.readLine())!=null)
            { 
                StringTokenizer tokens = new StringTokenizer(inputline, " ");
                
                while(tokens.hasMoreTokens())
                {
                    iline.add(tokens.nextToken());
                }
                
                LogIn nuevaconexion = new LogIn();
                boolean allowed = nuevaconexion.Conectar(iline.get(0).toString(), iline.get(1).toString());
